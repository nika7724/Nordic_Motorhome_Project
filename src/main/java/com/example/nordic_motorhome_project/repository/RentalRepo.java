...
public class RentalRepo {
    ...
    public Rental addRental(Rental rental) {
        String sql = "INSERT INTO rentals(id, motorho_id, start_date, end_date, season_id, location_id, item_id) values(?, ?, ?, ?, ?,?,?)";
        jdbcTemplate.update(sql, rental.getId(), rental.getMotorho_id(), rental.getStart_date(), rental.getEnd_date(), rental.getSeason_id(),
                rental.getLocation_id(), rental.getItem_id());
        return null;
    }
    ...
    public Rental findRentalById(int id) {
        String sql = "SELECT rental_id, c.id, c.first_name, c.last_name, m.motorho_id, " +
                ...
                "JOIN accessories a ON r.item_id = a.item_id JOIN fuel f ON r.fuel_id = f.fuel_id JOIN speed_meter p ON r.speed_meter_id = p.speed_meter_id WHERE rental_id =?";
        RowMapper<Rental> rowMapper = new BeanPropertyRowMapper<>(Rental.class);
    ...
    public Rental updateRental(int id, Rental rental) {
        String sql = "UPDATE rentals SET motorho_id =?, " +
                ...
                "location_id=?, item_id =? WHERE rental_id =?";
        jdbcTemplate.update(sql,rental.getMotorho_id(), rental.getStart_date(), rental.getEnd_date(), rental.getSeason_id(),rental.getLocation_id(), rental.getItem_id(), rental.getRental_id());
        return null;
    }

    public Rental chargeRental(int id, Rental rental) {
        String sql = "UPDATE rentals SET fuel_id=?, speed_meter_id=? WHERE rental_id =?";
