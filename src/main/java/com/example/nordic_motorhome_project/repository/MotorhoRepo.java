...
public class MotorhoRepo {
    ...
    public List<Motorhome> fetchAll() {
        String sql="SELECT motorho_id, motorho_name, size, availability FROM nordic_motorhome_project.motorhomes";
        RowMapper<Motorhome> rowMapper = new BeanPropertyRowMapper<>(Motorhome.class);
        return jdbcTemplate.query(sql,rowMapper);
    }

       public Motorhome addMotorho(Motorhome motorho) {
           String sql ="INSERT INTO nordic_motorhome_project.motorhomes(motorho_name, brand_name, size, year, season_id, availability) values(?, ?, ?, ?, ?, ?)";
...
public Motorhome findMotorhoById(int id) {
    ...
    String sql ="SELECT motorho_id, motorho_name, brand_name, size, year, s.season_id, s.price * 1.6 as price_peak, s.price * 1.3 as price_middle, s.price as price_low, availability FROM nordic_motorhome_project.motorhomes m JOIN nordic_motorhome_project.season_prices s ON m.season_id = s.season_id WHERE motorho_id =?";
    RowMapper<Motorhome> rowMapper = new BeanPropertyRowMapper<>(Motorhome.class);
...
public Boolean deleteMotorho (int id) {
    ...
    public Motorhome updateMotorho(int id, Motorhome motorho) {
        String sql ="UPDATE nordic_motorhome_project.motorhomes SET motorho_name =?, brand_name=?, size=?, year=?, season_id =?, availability=? WHERE motorho_id =?";
